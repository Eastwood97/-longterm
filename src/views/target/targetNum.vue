<template>
  <div class="app-container">
    ...
    <!-- 添加或修改对话框 -->
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" :close-on-click-modal="false" :show-close="false">
      <el-form
        ref="dataForm"
        :rules="rules"
        :model="dataForm"
      ...
      >
        <el-form-item prop="targetId" v-if="false">
          <el-input v-model="dataForm.targetId" />
        </el-form-item>
        <el-form-item label="目标名称" prop="targetName" :error="errorMsg">
          <el-input v-model="dataForm.targetName" @input="changeName" />
          <div class="el-form-item__error" v-if="this.errorMsg">名称已存在</div>
        </el-form-item>

        <el-form-item label="imsi" prop="imsi">
          <el-input v-model="dataForm.imsi" maxlength="15" />
        </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        >确定</el-button>
        <el-button v-else type="primary" @click="updateData" :disabled="disableFalg">确定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  data() {
    return {
      ...
      rules: {
        ...
        imsi:[
          {required:true,message:"imsi不能为空",tigger:"blur"},
          {required:true,pattern:/(^[\0-9][0-9]*([0-9]+)?)$/,message:"请输入数字",tigger:"blur"},
        ]
      },
  ...
  methods: {
    ...
    changeName(val) {
      verifyUnique(this.dataForm)
        .then(response => {
          if (response.data.errno == 509) {
    ...
    createData() {
      this.$refs["dataForm"].validate(valid => {
        if (valid && (this.errorMsg == null|| this.errorMsg == "")) {
          this.disableFalg = true;
